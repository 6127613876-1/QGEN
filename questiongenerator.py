...
import torch
import spacy
from spacy.lang.en import English  # Import English tokenizer from spaCy
spacy_nlp = English()  # Load English tokenizer
...
def print_qa(qa_list: List[Mapping[str, Any]], show_answers: bool = True) -> None:
...
if __name__ == "__main__":
    ...
    print("Enter your article text below (press Enter twice to finish):\n")
    article_lines = []
    while True:
        line = input()
        if not line.strip():
    ...
    article_text = "\n".join(article_lines)
    qa_pairs = question_generator.generate(article_text)
    print_qa(qa_pairs)
